...
const embedPath = path.resolve('src/constants/embeds.toml');
const ticketPath = path.resolve('src/constants/tickets.toml');
...
const embedConfig: {
	menu: APIEmbed;
} = toml.parse(fs.readFileSync(embedPath, 'utf-8'));
const ticketConfig = toml.parse(fs.readFileSync(ticketPath, 'utf-8'));
...
class SendMenu {
	...
	async sendMenu(interaction: CommandInteraction) {
		...
		Object.keys(ticketConfig).forEach((k: string) => {
			const ticketType: TicketType = ticketConfig[k];
	...
	async handle(interaction: StringSelectMenuInteraction): Promise<unknown> {
		...
		const ticketInformation: TicketType = ticketConfig[ticketValue];
	...
	async handleForm(interaction: ModalSubmitInteraction): Promise<void> {
		...
		const ticketInformation: TicketType = ticketConfig[ticketType];
